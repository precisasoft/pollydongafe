...
public class RegistradorUsuarioBean implements RegistradorUsuarioRemote {
	...
	public boolean registrarUsuario(String userName, String password,
		...
		if (stringTokenizer.hasMoreTokens()) {
			...
			if (dominio != null) {
				...
				if (!listadoEntidad.isEmpty()) {
					Entidad entidac = em.find(Entidad.class, listadoEntidad
							.get(0).getId());
					if (entidac.isHabilitado()) {
						if (((entidac.getUsuarioAdministrador() == null) || (entidac
								.getUsuarioAdministrador().length() < 2))) {
						...
						// empatar para que pueda ser el operador del
						// puntodeventa por defecto.

						Query qestablecimiento = em
								.createQuery("select e from Establecimiento e where e.entidad=?1");
						qestablecimiento.setParameter(1, entidac);
						List<Establecimiento> listadoEstablecimiento = qestablecimiento
								.getResultList();
						...
						for (Establecimiento establecimiento : listadoEstablecimiento) {
							...
							for (PuntoVenta pos : establecimiento.getPos()) {
								if (pos.isDefaultPuntoVentaElectronico()) {
									encontrado = true;
									PuntoVenta _pos = em.find(PuntoVenta.class,
											pos.getId());
									if ((_pos.getUsuarioPorDefecto() == null)
											|| (_pos.getUsuarioPorDefecto()
													.length() < 2)) {
		...
		Query qu=em.createQuery("select u from User u where u.username=?1");			
		qu.setParameter(1, userName);		
		List<User>listaUsuarios=qu.getResultList();
	...
	public Map<String, String> llenarUserInfo(String userName) {
		...
		if (stringTokenizer.hasMoreTokens()) {
			...
			if (dominio != null) {
				Query qEntidad = em
						.createQuery("select e from Entidad e where e.dominioInternet=?1");
				...
				if (!listadoEntidad.isEmpty()) {
					...
					mapa.put("razonSocialEmisor", listadoEntidad.get(0)
							.getRazonSocial());
					Query q = em
							.createQuery("select p from PuntoVenta p where p.usuarioPorDefecto=?1");
					q.setParameter(1, userName);
					List<PuntoVenta> listaPuntoVenta = q.getResultList();
					if (!listaPuntoVenta.isEmpty()) {
						PuntoVenta pos = em.find(PuntoVenta.class,
								listaPuntoVenta.get(0).getId());
						if (pos != null) {
							Establecimiento establecimiento = pos
									.getEstablecimiento();
							mapa.put("idEstablecimiento",
									String.valueOf(establecimiento.getId()));
							mapa.put("codigoEstablecimiento",
									establecimiento.getCodigo());
							mapa.put("puntoEmision", pos.getCodigoPuntoVenta());
							mapa.put("idPuntoEmision",
