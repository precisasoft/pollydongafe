	...
	public String recibirFactura(@WebParam(name = "factura") FacturaBinding factura) {
		...
		if (!lista.isEmpty()) {
			...
			if (entidad.isObligadoContabilidad()) {
				facturaxml.getInfoFactura().setObligadoContabilidad(
						ObligadoContabilidad.SI);
				if (entidad.getResolucionContribuyenteEspecial() != null) {
					facturaxml.getInfoFactura().setContribuyenteEspecial(
							entidad.getResolucionContribuyenteEspecial());
				}
			} else {
				facturaxml.getInfoFactura().setObligadoContabilidad(
			...
			facturaxml.getInfoFactura().setIdentificacionComprador(
					factura.getIdentificacionBeneficiario());
			facturaxml.getInfoFactura().setTipoIdentificacionComprador(
					factura.getCodigoTipoIdentificacionBeneficiario());
			facturaxml.getInfoFactura().setRazonSocialComprador(
					factura.getRazonSocialBeneficiario());
			...
			for (FacturaDetalleBinding d : factura.getDetalles()) {
				...
				detalle.setCantidad(d.getCantidad().setScale(4,
				...
				detalle.setPrecioTotalSinImpuesto(d
						.calcularValorTotalSinImpuestos());
