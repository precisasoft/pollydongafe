...
import ec.com.vipsoft.ce.backend.service.ConsultaComprobantesEmitidoBean;
...
public class ReceptorFacturaNeutra implements ReceptorFacturaNeutraRemote {
	...
	@EJB
	private ConsultaComprobantesEmitidoBean consultaPreviamenteEnviado;
	/*
	 * (non-Javadoc)
	 * 
	 * @see ec.com.vipsoft.ce.services.recepcionComprobantesNeutros.
	 * ReceptorFacturaNeutraRemote
	 * #recibirFactura(ec.com.vipsoft.ce.comprobantesNeutros.FacturaBinding)
	 */
	@Override
	@WebMethod
	...
	public String recibirFactura(@WebParam(name = "factura") FacturaBinding factura) {
		...
		//verificar si ya se envio dihco comprobante.
		if(factura.getSecuenciaDocumento()!=null){
			StringBuilder sbnumeroestabptovesecu=new StringBuilder();
			sbnumeroestabptovesecu.append(factura.getCodigoEstablecimiento());
			sbnumeroestabptovesecu.append("-");
			sbnumeroestabptovesecu.append(factura.getCodigoPuntoVenta());
			sbnumeroestabptovesecu.append("-");
			sbnumeroestabptovesecu.append(factura.getSecuenciaDocumento());
			ReporteComprobanteElectronico consultaFactura = consultaPreviamenteEnviado.consultaFactura(factura.getRucEmisor(), sbnumeroestabptovesecu.toString());
			if(consultaFactura!=null){
				if(consultaFactura.getEnProduccion()){
					if(consultaFactura.getAutorizacion()!=null){
						continuar=false;
						claveAcceso=consultaFactura.getClaveAcceso();
					}
					if(consultaFactura.getClaveAcceso()!=null){
						continuar=false;
						claveAcceso=consultaFactura.getClaveAcceso();
					}
					if(consultaFactura.getEnviadoSRI()){
						continuar=false;
						claveAcceso=consultaFactura.getClaveAcceso();
					}	
				}	
		...
		if(continuar){
			String rucEmisor = cryptoUtil.decrypt(factura.getRucEmisor());
			...
			if (!lista.isEmpty()) {
				...
				facturaxml.getInfoTributaria().setDirMatriz(limpiarDeEnter(entidad.getDireccionMatriz()));
				String nombreComercial=entidad.getNombreComercial();
				if(nombreComercial==null){
					nombreComercial=entidad.getRazonSocial();
				}
				facturaxml.getInfoTributaria().setNombreComercial(limpiarDeEnter(nombreComercial));
				facturaxml.getInfoTributaria().setRazonSocial(limpiarDeEnter(entidad.getRazonSocial()));
				facturaxml.getInfoTributaria().setRuc(cryptoUtil.decrypt(entidad.getRuc()));
				...
				facturaxml.getInfoFactura().setDireccionComprador(limpiarDeEnter(factura.getDireccionBeneficiario()));
				facturaxml.getInfoFactura().setDirEstablecimiento(limpiarDeEnter(entidad.getDireccionMatriz()));
				...
				facturaxml.getInfoFactura().setIdentificacionComprador(limpiarDeEnter(factura.getIdentificacionBeneficiario()));
				facturaxml.getInfoFactura().setTipoIdentificacionComprador(limpiarDeEnter(factura.getCodigoTipoIdentificacionBeneficiario()));
				facturaxml.getInfoFactura().setRazonSocialComprador(limpiarDeEnter(limpiarDeEnter(factura.getRazonSocialBeneficiario())));
				...
				try {
					...
					Object[] datosfirmados = firmaXml.signFile(certificado,
							dataToSign, privateKey, proveedor);
					...
					parametros.put("documentoOriginal",facturaxml);
	...
	public String limpiarDeEnter(String _texto){
